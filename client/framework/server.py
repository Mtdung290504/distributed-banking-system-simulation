...
class RPCSkeleton:
    ...
    def _validate_hash(self, client_hash: str, method_name: str):
        ...
        if client_hash != self._expected_hash:
            raise ValueError(
                f"Interface mismatch khi gọi '{method_name}'!\n"
                f"Client hash: {client_hash}\n"
                f"Server hash: {self._expected_hash}\n"
                f"Đảm bảo client và server dùng CHÍNH XÁC cùng interface!"
            )
...
def skeleton(service_class: Type[T], interface_class: Type[T]) -> RPCSkeleton:
    ...
    print(f"Server skeleton interface [{interface_class.__name__}] hash: {expected_hash}")
